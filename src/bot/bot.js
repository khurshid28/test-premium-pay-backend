  ...
  if (
    ...
    if (msg.text == "/data") {
      ...
      zayavkalar5 = await new Promise(function (resolve, reject) {
        db.query(
          `SELECT count(id) from Zayavka WHERE  status='finished' and status='paid'`,
          function (err, results, fields) {
            console.log(err);
            if (err) {
              resolve(null);
      ...
      bot.sendMessage(
        ...
        zayavkalar5 = await new Promise(function (resolve, reject) {
          db.query(
            `SELECT count(id) from Zayavka WHERE  status='finished' and status='paid' and DATE(now() - INTERVAL 5 HOUR)=DATE(created_time - INTERVAL 5 HOUR);`,
            function (err, results, fields) {
              console.log(err);
              if (err) {
                resolve(null);
      ...
      zayavkalar5 = await new Promise(function (resolve, reject) {
        db.query(
          `SELECT count(id) from Zayavka WHERE  status='finished' and status='paid' and  DATE(now() - INTERVAL 5 HOUR) - 1 = DATE(created_time - INTERVAL 5 HOUR)`,
          function (err, results, fields) {
            console.log(err);
            if (err) {
              resolve(null);
